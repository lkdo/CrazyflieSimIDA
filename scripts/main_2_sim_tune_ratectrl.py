...
freq_ctrl_angle = 1 # Flight Stab (not used)
dt_sim = 1.0/800.0  # integration step; fz has to be bigger than freq_ctrl_rate
...
while readkeys.exitpressed is False :
    ...
    if abs(t/att_controller.dt_ctrl_rate - round(t/att_controller.dt_ctrl_rate)) < 0.000001 :
        ...
        omegab_ref[0] = readkeys.ref[1]
        omegab_ref[1] = readkeys.ref[2]
        omegab_ref[2] = readkeys.ref[3]
    ...
    if abs(t/dt_vis - round(t/dt_vis)) < 0.000001 :
        ...
        panda3D_app.screenText_ref(np.array([thrust_ref,omegab_ref[0],omegab_ref[1],omegab_ref[2]]))
        
    # Logging frequency    
    if abs(t/dt_log - round(t/dt_log)) < 0.000001 :
        logger.log_attstab(t,np.array([0,0,0]),
