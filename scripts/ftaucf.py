...
class QuadFTau_CF:
    ...
    def __init__(self, std_ratio, plus = True, sigma = np.zeros(4) ):
    ...
    def input2thrust_i(self, cmd_i):
        ...
        if cmd_i < 1000:
            return 0
        return self.input2thrust_poly(cmd_i)+np.random.normal(0,self.sigma[0])
    
    def thrust2torque_i(self, ft_i):
        """ the torque [Nm] generated by a rotor yielding a given thrust [N] """
        if ft_i == 0:
           return 0
        return self.thrust2torque_poly(ft_i)+np.random.normal(0,self.sigma[1])
    ...
    def input2omegar_i(self, cmd_i):    
        ...
        if cmd_i <= 1000:
            ...
            return self.input2omegar_poly(cmd_i)+np.random.normal(0,self.sigma[2])
    ...
    def f_aero(self, omegar, vb):
      return np.dot(self.Kaero,vb)*np.sum(omegar) + np.random.normal(0,self.sigma[3])
